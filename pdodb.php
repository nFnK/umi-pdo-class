...
class Database
{
    ...
    public static function connect()
    {
        ...
        if (null == self::$cont)
        {
            ...
            {
                die("Hata bağlantı yok " /*.$e->getMessage()*/);
            }
        }
        return self::$cont;
    }
...
class CRUD extends Database
{
   ...
   public function insert($table,$params=array()){
    ...
    public function update($table,$id,$params=array()){
     $ikey = ''; foreach($params as $zzm => $valz) {$ikey .= $zzm." = :".$zzm.", ";} $ikey = rtrim($ikey,", ");   
   ...
   return $query->rowCount();
    ...
    {
        $query = $this->db->prepare("DELETE FROM deneme WHERE id = :id");
    ...
    {
        $query = $this->db->prepare("SELECT * FROM deneme WHERE id = :id");
        $query->bindParam("id", $user_id, PDO::PARAM_STR);
        $query->execute();
